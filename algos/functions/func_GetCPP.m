...
for k=1:length(F0)
    ...
    yseg_c = real(yseg_c); % added for octave compatibility
    % (in matlab, ifft would return a vector of reals in this situation
    % but octave doesn't)

    yseg_c_db = 10*log10(yseg_c .^ 2); % displays different from matlab but same vals
  
    yseg_c_db = yseg_c_db(1:floor(length(yseg)/2)); % as above
    [vals, inx] = func_pickpeaks(yseg_c_db(N_ms:end)', 2*N0_curr); % <==
    % octave: vals = [](1x0), inx = [](1x0)
    % matlab: vals = -20.1656, inx = 79
    %vals
    %inx
    %break;
