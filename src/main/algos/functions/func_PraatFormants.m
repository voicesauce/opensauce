...
elseif (ismac) % mac osx can run Praat using terminal, call Praat from
                % Nix/ folder
% KS wavfile already represents full path to file -- just pass that in directly instead                
%   curr_dir = pwd;
...
%   '%s %.3f %.3f %d'], curr_wav, frameshift, windowlength, maxFormant);
  ...
  cmd = sprintf(['src/main/praat/MacOS/Praat src/main/praat/Windows/praatformants.praat ' ...
  '%s %.3f %.3f %d'], wavfile, frameshift, windowlength, maxFormant);
